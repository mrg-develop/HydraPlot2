			...
			using ( Pen pen = source.GetPen( pane, scaleFactor ) )
			{
				if ( points != null && !_color.IsEmpty && this.IsVisible )
				{
					...
					for ( int i = 0; i < points.Count; i++ )
					{
                        ...
                        //Console.WriteLine(lastPt.X + " => " + curPt.X );
						...
						else
						{
                            ...
                            // PATCH: don't draw non existant points - mrioseco
                            // FIXME: que esto sea válido solo para plots de fecha y hora
                            // FIXME: buscar una forma más limpia de hacer esto (mejor insertar gaps en la DB) pero
                            // hay que implementar una lógica.
                            isOut = (tmpX < minX && lastX < minX) || (tmpX > maxX && lastX > maxX) ||
                                (tmpY < minY && lastY < minY) || (tmpY > maxY && lastY > maxY) ||
                                //(Math.Abs(curPt.X - lastPt.X) >= 1); // <-- patch hacer valor variable
                                (Utility.MinuteSpan(DateTime.FromOADate(curPt.X), DateTime.FromOADate(lastPt.X)) > 60); // PATCH: gap mayor que una hora => out
