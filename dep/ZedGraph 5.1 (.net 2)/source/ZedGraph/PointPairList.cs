...
*/
	...
	#region Methods
		...
		*/
		...
		private double _interpolateX( double xTarget, bool nullMissing, out bool isMissing )
		{
			int lo, mid, hi;
			if ( this.Count < 2 )
            ...
            interpolation = (xTarget - this[lo].X) / (this[hi].X - this[lo].X) *
            ...
            {
                if (Utility.MinuteSpan(DateTime.FromOADate(this[hi].X), DateTime.FromOADate(this[lo].X)) > 60)
                {
                    //Console.WriteLine("INTERPOLATION GAP detected");
                    interpolation = 0;
                    isMissing = true;
                }
                else
            ...
            //Console.WriteLine(this[hi].X + ", " + this[lo].X);
        ...
        {
            bool isMissing;
            return _interpolateX(xTarget, false, out isMissing);
        }

        public double? InterpolateXWithoutMissing(double xTarget)
        {
            bool isMissing;
            double interpolation = _interpolateX(xTarget, true, out isMissing);
            if (isMissing)
            {
                return null;
            }
        ...
        // FIX para mostrar gráficos apilados
