...
namespace ZedGraph
{
	...
	#region Private Fields
        ...
        // PATCH: para tener regla horizontal y vertical
        /*private VRuler _vruler;
        private HRuler _hruler; // aun no hecha
         * */
        private Rulers _rulers;
	...
	#region Constructors
			...
			: base( rhs )
		{
            ...
            // FIXME: pendiente implementar métodos para copia de las rulers
...
//			minSpaceT = Math.Max( minSpaceT, spaceT );
    ...
    #region Ruler stuff
        ...
        public Ruler AddVRuler()
        {
            // agregar rulers si aplica
            VRuler r = new VRuler(_chart._rect.X, _chart._rect.Y);
            //VRuler r = new VRuler(_rect.X, _rect.Y);
            _rulers.AddRuler(r);
            return r;

        }

        public void RemoveRuler(VRuler instance)
        {
            _rulers.RemoveRuler(instance);
        }

        public void RemoveRuler(HRuler instance)
        {
            _rulers.RemoveRuler(instance);
        }

        public void GetValuedXForRuler(VRuler instance)
        {
            XAxis x =  _xAxis;
        ...
        internal double GetXDelta(VRuler vRuler)
        {
            float firstRulerXPos = float.MaxValue;
            VRuler firstRuler = null;

            foreach (VRuler ruler in _rulers.VRulers)
            {
                if (ruler.GXPos < firstRulerXPos)
                {
                    firstRulerXPos = ruler.GXPos;
                    firstRuler = ruler;
                }
            }
            ...
            if (firstRuler != null)
            {
                delta = XAxis.Scale.ReverseTransform(vRuler.GXPos) - XAxis.Scale.ReverseTransform(firstRuler.GXPos);
            }

            return delta;
        }

        internal double GetYDelta(VRuler vRuler, CurveItem curve)
        {
            float firstRulerXPos = float.MaxValue;
            VRuler firstRuler = null;
            ...
            if (firstRuler != null)
            {
                ...
                Ruler.RulerIntersection intersection1 = GetIntersectionForCurve(firstRuler, curve);
                Ruler.RulerIntersection intersection2 = GetIntersectionForCurve(vRuler, curve);

                if (intersection1.yValue != null && intersection2.yValue != null)
                {
                    delta = ((double)intersection2.yValue) - ((double)intersection1.yValue);
                }
                else {
                    delta = 0;
                }
                //delta = YAxis.Scale.ReverseTransform(vRuler.GXPos) - XAxis.Scale.ReverseTransform(firstRuler.GXPos);
            }
        ...
        internal List<Ruler.RulerIntersection> GetYIntersections(VRuler vRuler)
        {
            //Graphics g = Graphics.FromHwnd( IntPtr.Zero );
            //PointF testPoint = new PointF();

            List<Ruler.RulerIntersection> result = new List<Ruler.RulerIntersection>(0);
            foreach (CurveItem curve in CurveList)
            {
                Ruler.RulerIntersection intersection = GetIntersectionForCurve(vRuler, curve);
        ...
        private Ruler.RulerIntersection GetIntersectionForCurve(VRuler vRuler, CurveItem curve)
        {
            PointPairList points = (PointPairList)curve.Points;
            //Console.WriteLine(curve.Label.Text + " Interpolation " + points.InterpolateXWithoutMissing(this.XAxis.Scale.ReverseTransform(vRuler.GXPos)));
            double? interpolated = points.InterpolateXWithoutMissing(this.XAxis.Scale.ReverseTransform(vRuler.GXPos));
            Ruler.RulerIntersection intersection = new Ruler.RulerIntersection();
            intersection.label = curve.Label.Text;
            intersection.color = curve.Color;
            intersection.gXpos = vRuler.GXPos;
            intersection.curve = curve;
            if (interpolated != null)
            {
                intersection.gYpos = this.YAxis.Scale.Transform((double)interpolated);
                intersection.yValue = interpolated - curve.Points.Bias;
            }
            else
            {
                intersection.gYpos = 0;
                intersection.yValue = null;
            }
            return intersection;
        }
    }
